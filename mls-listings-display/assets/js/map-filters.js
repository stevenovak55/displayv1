...
const MLD_Filters = {
    ...
    populateAmenityCheckboxes: function(counts) {
        ...
        container.find('label').each(function() {
            ...
            let countEl = $(this).find('.bme-filter-count');
            if (countEl.length === 0) {
                countEl = $('<span class="bme-filter-count"></span>');
                if ($(this).find('.bme-label-text').length === 0) {
                    const labelText = this.innerText.trim();
                    checkbox.after(`<span class="bme-label-text">${labelText}</span>`);
    ...
    renderFilterTags: function() {
        ...
        for(const key in modalFilters) {
            if(typeof modalFilters[key] === 'boolean' && modalFilters[key] === true) {
